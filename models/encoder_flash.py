...
class EncoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.layer_norm = MultiwayWrapper(args, LayerNorm(self.embed_dim)) 
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None, multiway_split_position=None):
        ...
        if multiway_split_position is not None:
            # 如果设置了多路分割位置
            assert self.args.multiway  # 确保启用了多路方式
            self.apply(set_split_position(multiway_split_position))  # 设置分割位置
...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        if multiway_split_position is not None:
            # 如果有多路分割位置
            assert self.args.multiway  # 断言模型配置支持多路分割
            self.apply(set_split_position(multiway_split_position))  # 应用多路分割位置
